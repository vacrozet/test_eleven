...
export function AstronautForm({
  ...
  useEffect(() => {
    if (currentPlanet !== "NO_WHERE" && currentPlanet?.isHabitable) {
      setAstronautOriginPlanet(currentPlanet.id.toString());
    }
  }, [currentPlanet]);

  useEffect(() => {
    if (mode === "edit" && astronautForUpdate) {
      ...
      <HUDWindow>
        ...
        >
          ...
          <HUDAutoComplete
            ...
            onChange={(e) => setAstronautOriginPlanet(e.value)}
          />
          <Flexbox
